...
Geometries::Prism::Prism(glm::vec3 origin, glm::vec3 gen1, glm::vec3 gen2, glm::vec3 gen3)
{
    ...
    points.push_back({origin,{0,1}});                      // 0
    points.push_back({origin + gen1,{1,1}});               // 1
    points.push_back({gen3 + gen1 - origin,{0,0}});        // 2
    points.push_back({origin + gen3,{1,0}});               // 3
    points.push_back({origin + gen2,{0,1}});               // 4
    points.push_back({gen1 + gen2 - origin,{1,1}});        // 5
    points.push_back({gen3 + gen2 + gen1 - origin,{0,0}}); // 6
    points.push_back({gen3 + gen2 - origin,{1,0}});        // 7
